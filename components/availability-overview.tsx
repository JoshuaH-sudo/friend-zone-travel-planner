...
export function AvailabilityOverview({ friends }: AvailabilityOverviewProps) {
  ...
  const getAvailableFriends = (date: Date) => {
    return friends.filter((friend) => friend.availableDates.some((d) => isSameDay(d, date)))
  }
      ...
      <CardContent className="p-3">
        ...
        <div className="grid grid-cols-7 gap-1 text-center">
          ...
          {monthDays.map((day) => {
            ...
            const allAvailable = availableCount === friends.length && friends.length > 0
              ...
              <TooltipProvider key={day.toString()}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div
                      className={cn(
                        ...
                        allAvailable && "bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800",
                      )}
                    >
                      <div className="text-sm font-medium self-end">{format(day, "d")}</div>

                      {availableCount > 0 && (
                        <div className="mt-auto flex flex-wrap gap-1 justify-center">
                          {availableCount === friends.length && friends.length > 0 ? (
                            <Badge className="bg-green-500 hover:bg-green-600">Everyone</Badge>
                          ) : (
                  ...
                  <TooltipContent side="bottom" align="center">
                    <div className="text-sm font-medium">{format(day, "EEEE, MMMM d, yyyy")}</div>
                    {availableCount > 0 ? (
                      <div className="mt-1">
                          ...
                          {availableFriends.map((friend) => (
                            <li key={friend.id} className="flex items-center gap-1">
                              <span
                                ...
                                style={{ backgroundColor: friend.color }}
                              />
                              {friend.name} <span className="text-xs text-muted-foreground">({friend.timezone})</span>
                            </li>
                          ))}
                        </ul>
                      </div>
