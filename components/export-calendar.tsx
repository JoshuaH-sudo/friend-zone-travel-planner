...
export function ExportCalendar({ friends, groupName }: ExportCalendarProps) {
  ...
  const exportAllCalendars = () => {
    ...
    const filename = groupName ? `${groupName.replace(/\s+/g, "_")}_availability.ics` : "all_friends_availability.ics"
    downloadFile(icalContent, filename)
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-lg font-medium mb-2">{t("actions.exportCalendars")}</h3>
        <p className="text-sm text-muted-foreground mb-4">{t("calendar.exportDescription")}</p>

        <div className="flex flex-wrap gap-2">
          <Button onClick={exportAllCalendars} disabled={friends.length === 0} className="gap-2">
      ...
      {friends.length > 0 && (
        <div>
          <h4 className="text-sm font-medium mb-2">{t("calendar.exportIndividual")}</h4>
