  ...
  if (!data?.hotels || data.hotels.length === 0) {
    return (
      <div className='text-muted-foreground py-8 text-center'>
        <p className='text-sm'>
          No accommodation options found for this location
        </p>
        <Button
          variant='outline'
          size='sm'
  ...
  return (
    <ScrollArea className='h-[300px] pr-4'>
      <div className='space-y-2'>
        <h3 className='mb-2 text-sm font-medium'>Select accommodation:</h3>
          ...
          {data.status === 'OK' &&
            data.hotels.map((hotel, index) => (
              <div key={index} className='flex items-start space-x-2'>
                ...
                >
                  <Card
                    className={`w-full transition-shadow hover:shadow-md ${selectedAccommodation?.name === hotel.name ? 'border-primary' : ''}`}
                  >
